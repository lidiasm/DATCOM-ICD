...
friedman.test(data.matrix(clasif_models %>% dplyr::select(-X)))
```
Finalmente, como el test de Friedman ha rechazado la hipótesis nula, vamos a aplicar un Post Hoc para comparar a todos los algoritmos entre sí realizando **pares de tests (LDA vs. QDA, QDA vs. KNN, LDA vs. KNN)**. Para cada una de las combinaciones utilizaremos el **test de Wilcoxon** de modo que devuelva los p-valores. Sin embargo, la sucesión de múltiples tests sobre el mismo conjunto de datos puede disparar el error `FWER`. Por ello, aplicaremos la **penalización por pasos mediante Holm** para ajustar los p-valores obtenidos y así controlar el error acumulado.Como podemos comprobar en los resultados, los p-valores devueltos para cada una de las comparaciones son altísimos, lo cual indica que estadísticamente no existe ningún algoritmo que destaque sobre los demás.
