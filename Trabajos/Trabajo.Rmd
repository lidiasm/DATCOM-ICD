...
4. Las variables relativas a las diferentes medidas de peso disponen de un rango más amplio de valores, con una **concentración de ejemplares de poco peso**. Este hecho me resulta extraño puesto que, como se ha comentado anteriormente, el tamaño de sus conchas es considerablemente grande, por lo que **cabría esperar que sus pesos también fueran mayores**. Por otro lado, creo que pueden existir las siguientes relaciones entre este tipo de predictores.
    ...
    * Las variables `Shucked_weight` y `Viscera_weight` pueden estar relacionadas linealmente puesto que me parece lógico pensar que si **aumenta el peso de los órganos de un ejemplar, también aumente de peso el propio animal**. De igual modo, esta hipótesis podrá comprobarse mediante un gráfico de correlaciones.
  
5. Finalmente, cabe destacar que parece haber una **mayor representación de ejemplares jóvenes que de otras edades**. Este hecho puede suponer un problema en caso de que las características físicas varíen en función de la edad, puesto que si existen pocos ejemplares de mayor edad, los modelos pueden no aprender suficientemente bien los rasgos característicos de cada generación como para identificar a las futuras muestras más mayores.
...
Para finalizar este estudio multivariante, procedemos a representar un diagrama de cajas con todas las variables numéricas para visualizar, de manera gráfica, la dispersión de los datos y comprobar si concuerda con las conclusiones extraídas anteriormente. Tal y como podemos apreciar, la mayoría de predictores tienen una longitud mínima, lo que supone que se encuentran caracterizados por una **ínfima variabilidad de sus valores**. Asimismo, también podemos observar que la mayoría dispone de un **intervalo de valores muy similar**, por lo que no existen variables con escalas predominantes que puedan afectar a los modelos. Otra conclusión que puede extraerse de este gráfico es que todos los predictores representados contiene **outliers moderados**, puesto que se encuentran muy cercanos al conjunto de valores de cada uno, por lo tanto no se trata de valores extremos. 

Sin embargo, en el caso de la variable independiente `Ring` podemos observar como su **dispersión es mayor** puesto que su correspondiente caja dispone de una mayor longitud. Esto nos indica que sus valores son más diversos, conclusión a la que habíamos llegado anteriormente. Del mismo modo, también se confirma la teoría de que existe una **concentración de ejemplares jóvenes** de una edad entre 5 y 15, es por ello por lo que aparece un mayor número de outliers en las restantes franjas de edad. Observándolos, podemos contar que el segundo grupo más representado es el que tiene una edad a partir de 15, y los que menos aparecen son aquellos que se encuentran en el intervalo [0, 5].
...
En este segundo apartado se pretende construir diferentes modelos aplicando diversas técnicas de Regresión para intentar maximizar el número de aciertos en la tarea de predecir la variable `Ring` en este dataset.

### 1.4.1. Regresión Lineal Simple

Comenzamos utilizando la Regresión Lineal Simple entrenando un modelo distinto para cada una de las cinco mejores variables identificadas. Según los estudios realizados anteriormente, la variable **`Shell_weight` es la que dispone de un coeficiente de correlación mayor con respecto `Ring`**. Por lo tanto, a continuación construimos un primer modelo con esta variable en solitario. 
...
summary(abalone.ls1)
```
Como podemos apreciar en los resultados, el p-valor del predictor es menor que el umbral de 0.05 y además dispone de un alto nivel de significación, por lo que **la variable `Shell_weight` resulta útil para la predicción de `Ring`**. Sin embargo, el valor de R² ajustado indica que este modelo solo es capaz de explicar un 39% aproximadamente de los datos, mientras que el modelo dispone de un RSE de 2.51, por lo tanto parece que será necesario añadir más variables para mejorar su capacidad de generalización.
...
summary(abalone.ls2)
```
Como podemos apreciar en los resultados, son muy similares a los del modelo anterior, puesto que el **nivel de significación estadística del predictor `Diameter` es muy alto. Sin embargo, en este modelo el RSE aumenta ligeramente mientras que el valor de R² ajustado disminuye hasta un 33%**. Este hecho nos puede indicar que la variable `Diameter` también proporciona información útil para predecir la variable independiente `Ring` aunque en menor medida que la variable `Shell_weight`.

Para el tercer modelo vamos a utilizar la variable `Length`, que como hemos observado anteriormente, está fuertemente correlada con `Diameter` y con la variable independiente. Una forma de conocer **cuál de los dos predictores proporciona mayor información útil** puede ser entrenar un modelo con cada uno para luego comparar sus resultados.
...
summary(abalone.ls3)
```
En comparación con el modelo anterior, podemos observar que este tercer **modelo entrenado con `Length` dispone de un mayor RSE y un valor de R² ajustado**, lo que nos indica que esta variable aporta menor información para la predicción de la variable independiente. Por lo tanto, en caso de tener qué decidir qué predictor sería el más conveniente, en función de los resultados se escogería la variable `Diameter`.

En se pretende utilizar una de las **variables más diferentes del dataset: el predictor `Height`**. Como se ha observado en los diferentes análisis, esta variable dispone de una distribución más extraña, comparada con el resto de predictores, y aporta información general al tamaño del ejemplar considerando tanto el animal como su concha.
...
summary(abalone.ls4)
```
Tal y como se aprecia en el resumen de este tercer modelo, el predictor `Height` no mejora ni la explicabilidad de la variable independiente ni el RSE, incluso se puede observar un **ligero empeoramiento de las medidas** respectivas. Este hecho nos indica que aporta menos información útil para la predicción de `Ring` que en los otros casos anteriores.
...
summary(abalone.ls5)
```
Tal y como se comentaba anteriormente, **esta variable no está tan fuertemente asociada** a la variable independiente y por ello su RSE aumenta, mientras que el valor del R² ajustado disminuye hasta situarse por debajo del 30%. Por lo tanto, tampoco es suficiente como para explicar toda la variabilidad de los datos.

A continuación se detallan las conclusiones que se pueden extraer de este apartado de modelos con Regresión Lineal Simple.

1. **El modelo con un menor RSE y un mayor valor de R² ajustado ha sido el primero en el que intervenía únicamente la variable `Shell_weight`**, la cual estaba más fuertemente correlada con la variable independiente `Ring`. Por lo tanto, se trata de un predictor que aporta suficiente información como para alcanzar por sí solo casi un 40% de explicabilidad de los datos.

2. Pese a la extraña distribución y a su menor grado de correlación con la variable independiente, el **predictor `Height` es el que ha proporcionado el segundo mejor modelo** con un 31% de explicabilidad y un RSE de 2.67. Este hecho nos indica que la altura puede identificar a un tercio de los ejemplares de manera independiente. Con una alta probabilidad, estos animales pertenecerán a la categoría de infantes, los cuales es lógico que dispongan de un menor tamaño que los adultos.

3. Finalmente, en modelos sucesivos en los que participen varios predictores, deberemos **eliminar la varaible `Length` puesto que se encuentra fuertemente relacionada con la variable `Diameter`**, y como hemos podido comprobar, este último predictor ha proporcionado un modelo con menor RSE y una mayor explicabilidad de la varaible independiente.

### 1.4.2. Regresión Lineal Múltiple

En este apartado se pretende construir diferentes modelos utilizando diversos predictores mediante Regresión Lineal Múltiple para intentar mejorar los resultados del mejor modelo obtenido con Regresión Lineal Simple. Para ello aplicaremos la **técnica backward** en el que comenzamos con un primer modelo en el que se incluyen todos los predictores, para después eliminar aquellos que no sean relevantes. 
...
En el resumen estadístico podemos observar que considerando todos los predictores hemos conseguido mejorar la explicabilidad de los datos aumentando el valor de R² ajustado hasta un 53%, además de haber reducido el RSE. Sin embargo, podemos apreciar que el **predictor `Length` tiene asociado un p-valor más alto que el umbral** de 0.05, lo que nos indica que puede ser eliminado sin influir en el modelo. Este hecho ya se había previsto anteriormente si recordamos la fuerte correlación existente entre `Diameter` y `Length`. Por lo tanto, eliminamos esta variable y procedemos a entrenar y analizar un segundo modelo.
...
summary(abalone.lm4)
```
Como podemos apreciar, mientras que el RSE ha aumentado muy ligeramente el porcentaje de explicabilidad también se ha elevado un poco más. Esto nos indica que la **información proporcionada por la variable eliminada no es lo suficientemente importante** como para mantenerla en el modelo. Si bien ya disponemos de un modelo con razonablemente buenas métricas de calidad, creo que podemos simplificarlo aún más. Así, disponemos de un modelo con unas razonablemente buenas métricas de calidad y una mayor sencillez en el conjunto de predictores. 

A continuación procedemos a añadir **términos no lineales** para intentar mejorar su capacidad de generalización. En el quinto modelo se añade una **interacción entre las varaibles `Shell_weight` y `Diameter`**, con las que se han obtenido dos de los mejores modelos utilizando Regresión Lineal Simple. 
...
summary(abalone.lm6)
```
Realizando varios experimentos que no se incluyen en esta memoria para no extenderla demasiado, he podido comprobar que añadiendo términos cuadráticos también se consigue cierta mejora en la calidad del modelo, a costa de añadir complejidad. Un ejemplo representativo es el que se muestra en el séptimo modelo, en el que se añaden los **términos cuadráticos de dos variables asociadas al peso: `Shucked_weight` y `Whole_weight`**. Como se puede apreciar, el porcentaje de R² ajustado aumenta a un 55%, mientras que el RSE disminuye cuatro centésimas. 
...
knnMSEtest
```
Como podemos observar, el **error obtenido en el conjunto de entrenamiento es mayor que el del mejor modelo con Regresión Lineal Múltiple**. Esto demuestra que no siempre una misma formulación proporciona buenos resultados para algoritmos diferentes, como ha sido esta comparación entre KNN y Regresión Lineal Múltiple. 

## 1.5. Comparación de algoritmos

Para finalizar este apartado de regresión se pretende realizar una comparación entre el comportamiento de diversos algoritmos sobre diferentes datasets. En primer lugar, aplicamos la Validación Cruzada para entrenar **cinco modelos con Regresión Lineal Múltiple y otros cinco con el algoritmo KNN**, tal y como se realizó en la sección anterior, solo que en este caso **se consideran todos los predictores**. 
